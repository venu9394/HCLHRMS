 ...
 */
public class Lop_emp_fetching extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public Lop_emp_fetching() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int Record_cnt=0;
		  ...
		  Res=null;
		    ...
		    Res=null;
		    try {
				Res=(ResultSet)DataObj.FetchData("Select count(*) from  hclhrm_prod_others.tbl_emp_lop_arrear_payperiod where payment="+PayPeriod+" and  BUSINESSUNITID in("+GetBuIds_Result.toString()+") ", "INSERT_STATUS", Res ,conn);
				if(Res.next()){
		    ...
		    // While Start
		    	...
		    	GetEmployeeData.append(" select a.employeeid ,d.callname,a.employeesequenceno ,b.dateofjoin,a.businessunitid ,  ");
		    	GetEmployeeData.append(" DATEDIFF('"+Global.get(BuID+"_TD")+"','"+Global.get(BuID+"_FD")+"')+1 actualdays,DATEDIFF('"+Global.get(BuID+"_TD")+"' , if(b.dateofjoin>='"+Global.get(BuID+"_FD")+"',b.dateofjoin,'"+Global.get(BuID+"_FD")+"') )+1 Eligibledays , ");
		    	GetEmployeeData.append(" ifnull(c.lopcount,0) lopcount from hclhrm_prod.tbl_employee_payperiod_details a ");
		    	GetEmployeeData.append(" left join hclhrm_prod.tbl_employee_profile b on  b.employeeid=a.employeeid ");
		    	GetEmployeeData.append(" left join hclhrm_prod.tbl_employee_lop c on c.employeeid=b.employeeid ");
		    	GetEmployeeData.append(" left join hclhrm_prod.tbl_employee_primary d on ");
		    	GetEmployeeData.append(" d.employeeid=b.employeeid where a.payperiod="+PayPeriod+" and ");
		    	GetEmployeeData.append(" a.businessunitid in("+BuID+") and  c.employeeid=b.employeeid and a.businessunitid=c.businessunitid and a.payperiod=c.loptransactionid ");
		    	GetEmployeeData.append(" and d.status=1001 and c.lopcount!=0 and ");
		    	GetEmployeeData.append(" c.lopcount is not null ");
		...
		request.getRequestDispatcher("lop_arrear.jsp").forward(request, response);
	}
