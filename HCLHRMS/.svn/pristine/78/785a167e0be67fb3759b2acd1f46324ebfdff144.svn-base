 ...
 */
public class PF_UAN_Db_insert extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public PF_UAN_Db_insert() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    ...
	    Map BankN_Acc=new HashMap();
		  ...
		  request.setAttribute("GlobalMsg","PF & UAN Update Faild please contact admin.");
    ...
    //*******************************Execute Queries *******************************
		  ...
		  */
		  InserQuery.append(" insert into hclhrm_prod.TBL_EMPLOYEE_OTHER_DETAIL(EMPLOYEEID,PFNO,PFUAN,MODIFIEDBY,ESINO) ");
		  InserQuery.append(" values(?,?,?,?,?) ON DUPLICATE KEY UPDATE PFNO=?, PFUAN=?,ESINO=? ");
	...
	//******************************************************************************
		...
		for(int i=0 ;i<Data_Acc.length;i++){
		    ...
		    String ESIACC=request.getParameter(Data_Acc[i]+"_ES");
			
		    BankN_Acc.put(Data_Acc[i]+"_BN",ParamBankName);
		    BankN_Acc.put(Data_Acc[i]+"_BA",ParamBankAcc);
		    BankN_Acc.put(Data_Acc[i]+"_ES",ESIACC);
	...
	try{		
		    ...
		    while(empitr.hasNext()){
				...
				*/
			    ...
			    preparedStatement.setString(2, BankN_Acc.get(Param+"_BN").toString());
			    ...
			    preparedStatement.setString  (5, BankN_Acc.get(Param+"_ES").toString());
			    preparedStatement.setString  (6, BankN_Acc.get(Param+"_BN").toString());
			    preparedStatement.setString  (7, BankN_Acc.get(Param+"_BA").toString());
			    preparedStatement.setString  (8, BankN_Acc.get(Param+"_ES").toString());
			    preparedStatement.addBatch();
	...
	if(affectedRecords.length>0){
		 ...
		 request.setAttribute("GlobalMsg","PF & UAN Updated successfully...!");
		...
		request.setAttribute("GlobalMsg","PF & UAN  Update Faild please contact system admin.");
	}
