    ...
    $(document).ready(function(){
     ...
     <%-- var Data=JSON.parse('<%=EMP_LOPLIST%>'); --%>
    	 ...
    	 for(var i=0;i<Data.length;i++){
           ...
           var markup = "<tr><td><input type='checkbox' "+tempjason.CHECKBOX+" <%=Mode%> class='z' name='ChkBox' value='" + tempjason.EMPID + "' id='" + tempjason.SEQNO + "' >"+j+" </td> <td>" + tempjason.SEQNO + "</td>"
            +" <td>" + tempjason.CALLNAME + "</td><td>"+tempjason.DATEOFJOIN+" </td>"
            +"<td>"
            +" <input type='text' value='"+tempjason.BANKID+"'name='"+tempjason.EMPID+"_BN'  id='"+tempjason.EMPID+"' '<%=Mode%>'  onkeypress='return isNumber(event)' style='width:180px'   />"
           <%--  +" <select name='"+tempjason.EMPID+"_BN' id='"+tempjason.EMPID+"' style='width:160px' <%=Mode%> > "
            ...
            +" <option value='11' "+tempjason.BANKCK_11+">ANARA BANK</option>"
            +" </select>" --%>
            
            +"</td> "
            +"<td> <input type='text' value='"+tempjason.BANKACC+"' name='"+tempjason.EMPID+"_BA'   id='"+tempjason.EMPID+"' '<%=Mode%>'  onkeypress='return isNumber(event)' style='width:180px'   /></td> "
            +"<td> <input type='text' value='"+tempjason.ESINO+"' name='"+tempjason.EMPID+"_ES'   id='"+tempjason.EMPID+"' '<%=Mode%>'   style='width:160px'   /></td></tr>";
...
<body onload='SetGlobalMsg();'>
 ...
 <form action="PF_UAN_Db_insert" method='post' autocomplete='off' >
