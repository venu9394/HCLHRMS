 ...
 */
public class Arrear_lop_caliculation extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public Arrear_lop_caliculation() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
       ...
       try{
    	   ...
    	   String insertTableSQL ="insert into hclhrm_prod_others.tbl_emp_lop_arrear_payperiod(EMPLOYEEID,BUSINESSUNITID,PAYMENT,PAYPERIOD,PAYSTRUCTUREID,GROSSVALUE,BASEDAYS,PAYABLEDAYS,NOOFDAYS,COMPONENTID,ARREAR)"
       ...
       String EMPID=Empselect.next().toString();
          ...
          String LOPDAY=Emp_map.get(EMPID+"_LOP").toString();
          Double Temp= Double.parseDouble(LOPDAY);
          Double Temp1= Double.parseDouble(LOPDAY);
                 if(Temp1<0){
                	 Temp1=-(Temp1); 
                 }
     ...
     // Calling For Data
       ...
       if(callSelect || Routing.equals("REPORT")){
		   ...
		   Fetch_Record.append(" select b.employeesequenceno,b.employeeid,b.callname,d.name buname,c.name componentname,a.noofdays,a.arrear,a.componentid,a.payment from hclhrm_prod_others.tbl_emp_lop_arrear_payperiod a ");
    	   ...
    	   Res=null;
		    try {
				...
				while(Res.next()){
					...
					if(Double.parseDouble(Res.getString("noofdays"))<0){
					 FetchData_Map.put(Res.getString("employeesequenceno")+"_"+Res.getString("componentid"),   -(Double.parseDouble(Res.getString("arrear"))));
					}else{
		    ...
		    try{
    	    Filename=Report_lwp.Report_lop_Gen(linkedHashSet_title,linkedHashSet_employee,FetchData_Map, LOGINBY);
		    }catch(Exception er){
       ...
       request.getRequestDispatcher("lop_arrear_download.jsp").forward(request, response);	  
	}// Close Do post
